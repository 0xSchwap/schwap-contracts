...
contract veSCH is Ownable, ReentrancyGuard {
    uint256 private id;
    ...
    function initialize(address _SCH) external onlyOwner {
        require(!initialized);
        initialized = true;
        SCH = IERC20(_SCH);
        for (uint256 _i = 0; _i < 5; _i++) {
            Vault _vault = new Vault(_SCH);
            vaults[periods[_i]] = address(_vault);
    ...
    function createLocker(uint256 _amount, uint256 _period) external nonReentrant returns (uint256) {
        ...
        require(SCH.balanceOf(msg.sender) >= _amount && SCH.allowance(msg.sender, address(this)) >= _amount);
        uint256 balance = SCH.balanceOf(address(this));
        SCH.transferFrom(msg.sender, address(this), _amount);
        require(SCH.balanceOf(address(this)) == balance + _amount);
        ...
        votingPower += weights[_period] * _amount;

        Locker memory _locker = Locker(_id, msg.sender, address(0), block.timestamp, block.timestamp + periods[_period - 1], _amount, true, _period);
        lockers[_id] = _locker;
    ...
    function _refresh(address _voter, uint256 _period, bool _slash) private {
        ...
        if (_amount > 0) {
            if (_slash) {
                ...
                payable(msg.sender).call{value: _reward}("");
                payable(_voter).call{value: _amount - _reward}("");
            } else {
                payable(_voter).call{value: _amount}("");
            }
        }
    ...
    function _unlockSCH(uint256 _id, bool _slash) private {
        ...
        Locker memory _lockerNew = Locker(_id, _locker.minter, msg.sender, _locker.created, _locker.unlocks, _locker.amount, false, _locker.period);
        lockers[_id] = _lockerNew;
    ...
    function getVotingPower(address _voter) external view returns (uint256) {
        ...
        for (uint256 _i = 0; _i < _matches; _i++) {
            if (_array[_i].locked && _array[_i].unlocks > block.timestamp) _votingPower += periods[_array[_i].period - 1] * _array[_i].amount;
        }
        return _votingPower;
    }
    ...
    function getVault(uint256 _period) external view returns (address) {
        return (vaults[periods[_period]]);
    }

    function getVaults() external view returns (address, address, address, address, address) {
